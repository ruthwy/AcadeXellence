...
export default function StudentNavbar({ isSubMenu, wrapperClass, menuItemClass }: Props) {
  ...
  // useEffect(() => {
  //   fetch("https://axs2459.uta.cloud/profile.php", {
  //     credentials: "include", // This is important for session cookies to be sent with the request
  //   })
  //     .then((response) => response.json())
  //     .then((data) => {
  //       console.log(data);
  //       if (data.error) {
  //         // Handle not logged in error, redirect to login or show a message
  //       } else {
  //         // Update state with user data
  //         // Inside your useEffect after fetching the data
  //         setProfile({
  //           firstname: data.name,
  //           lastname: data.lname,
  //           email: data.email,
  //           phone: data.phone, // make sure 'phone' is the correct key from your response
  //           id: data.id, // make sure 'id' is the correct key from your response
  //         });
  //       }
  //     })
  //     .catch((error) => {
  ...
  const handleEditProfileSubmit = (event: React.FormEvent) => {
    ...
    formData.append('lastname', editProfile.lastname); // Assuming the backend will be updated to handle lastname
    formData.append('email', editProfile.email);
    formData.append('phone', editProfile.phone);

    fetch('https://pxm5627.uta.cloud/updateProfile.php', {
      method: 'POST',
      body: formData,
      credentials: 'include',
    })
      ...
      .then(data => {
        if (data.error) {
          ...
          setProfile(prev => ({
            ...prev,
            firstname: data.name,
            lastname: data.lname, // Assuming backend returns 'lname' after update
            email: data.email,
            phone: data.phone,
          }));
